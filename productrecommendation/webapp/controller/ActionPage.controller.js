sap.ui.define([
    ...
    "use strict";
    return BaseController.extend("com.knpl.dga.productrecommendation.controller.ActionPage", {
        formatter: formatter,
        onInit: function () {
        ...
        _uploadToolImage: function (oData) {
            ...
            if (this._action === "add") {
                this.oFileUploader.setUploadUrl(this.sServiceURI + "ProductRecommendations(" + oData.Id + ")/$value?doc_type=image&file_name=" + this.imageName);
            }
        ...
        _uploadPdf: function (oData) {
            ...
            if (this._action === "add") {
                ...
                catalogue.forEach(function (ele) {
                    ...
                    jQuery.ajax({
                        method: "PUT",
                        url: sServiceUri + "ProductRecommendations(" + oData.Id + ")/$value?doc_type=pdf&file_name=" + ele.fileName + "&language_code=" + ele.LanguageCode,
                        cache: false,
                        contentType: false,
                        processData: false,
                        data: ele.file,
        ...
        onImageView: function (oEvent) {
            ...
            if (!this._imageDialog) {
                Fragment.load({
                    name: "com.knpl.dga.productrecommendation.view.fragments.ImagePopup",
                    controller: this,
                }).then(
                    function (oDialog) {
